...
;; Problem 17
(define (nondecreaselist t)
    (if (null? (cdr t)) (list t)
        (let ((recur (nondecreaselist (cdr t))))
            (cond
                ((null? (cdr t)) (list t))
                ((> (car t) (cadr t)) (append (list (list (car t)))
                                              recur))
                (else (append (list (cons (car t) (car recur)))
                            (cdr recur)))
            )
        )
    )
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
           ...
           (cons 
              (cons 'lambda 
                (cons (map let-to-lambda (car (zip values)))
